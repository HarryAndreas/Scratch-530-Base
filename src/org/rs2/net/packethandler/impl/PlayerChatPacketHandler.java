 ...
 */
public class PlayerChatPacketHandler implements PacketHandler {
	...
	public void handlePacket(Message m, GameSession gs) {
		int chatEffect = m.readShort() & 0xFFFF;
		int length = m.readByte() & 0xFF;
		String msg = decryptPlayerChat(m, length);
		gs.getPlayer().getUpdateFlags().sendChat(new ChatMessage(chatEffect, msg));
	}
	
	/**
	 * A byte array for encrypting chat msg's
	 */
